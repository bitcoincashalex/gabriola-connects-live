...
// Version: 3.1.1 - Fixed: Fetch from users table instead of outdated view
// Date: 2024-12-13
...
export default function AdminUsersPage() {
  ...
  const fetchUsers = async () => {
    // Fetch directly from users table to get all current fields
    // (user_admin_stats view might be outdated and missing new admin fields)
  ...
  const updatePermission = async (userId: string, field: string, value: any) => {
    ...
    if (error) {
      console.error('[Admin] Database update failed:', error);
      alert(`Failed to save: ${error.message}\n\nThis might be a permissions issue. Check Supabase RLS policies for the users table.`);
      // Revert on error
      await fetchUsers();
    } else {
      console.log('[Admin] Database update successful');
      // Force a refresh after 1 second to confirm the save
