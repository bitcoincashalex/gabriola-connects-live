...
import { Search, Calendar, MessageSquare, MapPin, Ship, AlertTriangle, Filter, X, ChevronDown, ChevronUp, Ban } from 'lucide-react';
...
const isEventFree = (fees: string | null): boolean => {
  if (!fees) return true;
  const lowercaseFees = fees.toLowerCase();
  return lowercaseFees === 'free' || lowercaseFees === '0' || lowercaseFees === 'none' || lowercaseFees === 'n/a';
};

const isAlertActive = (alert: any): boolean => {
...
export default function CommunitySearchPage() {
  // Auth check - community search is protected like the forum
  const { user } = useUser();
  ...
  // CRITICAL: Block banned users from accessing community search
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
        <div className="max-w-md w-full bg-white rounded-2xl shadow-lg p-8 text-center">
          ...
          <p className="text-gray-600 mb-6 text-lg">
            You have been banned from the Gabriola Connects community area.
          </p>
          ...
          <p className="text-gray-500 text-sm mb-6">
            If you believe this is a mistake, please contact the site administrators.
          </p>
          <Link
            href="/"
            className="inline-block px-6 py-3 bg-gabriola-green text-white rounded-lg hover:bg-green-700 transition-colors"
          >
            Return to Home
          </Link>
        </div>
  ...
  const performSearch = async () => {
    ...
    try {
      ...
      if (activeModule === 'all' || activeModule === 'events') {
        ...
        if (query.trim()) {
          eventsQuery = eventsQuery.or(`title.ilike.${searchTerm},description.ilike.${searchTerm},location.ilike.${searchTerm},category.ilike.${searchTerm},recurrence_pattern.ilike.${searchTerm},additional_info.ilike.${searchTerm},venue_name.ilike.${searchTerm},organizer_name.ilike.${searchTerm},organizer_organization.ilike.${searchTerm},contact_email.ilike.${searchTerm},what_to_bring.ilike.${searchTerm},accessibility_info.ilike.${searchTerm}`);
        }
      ...
      if (activeModule === 'all' || activeModule === 'directory') {
        ...
        // Only apply search filter if there's a search query
        if (query.trim()) {
          directoryQuery = directoryQuery.or(`name.ilike.${searchTerm},description.ilike.${searchTerm},category.ilike.${searchTerm},subcategory.ilike.${searchTerm},services.ilike.${searchTerm},specialties.ilike.${searchTerm},tagline.ilike.${searchTerm},owner_name.ilike.${searchTerm},address.ilike.${searchTerm},location_notes.ilike.${searchTerm}`);
        }
      ...
      if (activeModule === 'all' || activeModule === 'alerts') {
        ...
        // Only apply search filter if there's a search query
        if (query.trim()) {
          alertsQuery = alertsQuery.or(`title.ilike.${searchTerm},message.ilike.${searchTerm},category.ilike.${searchTerm},on_behalf_of_name.ilike.${searchTerm},on_behalf_of_organization.ilike.${searchTerm},contact_info.ilike.${searchTerm},action_required.ilike.${searchTerm}`);
        }
  ...
  const filterResults = () => {
    ...
    if (activeModule === 'all' || activeModule === 'alerts') {
      filtered.alerts = filtered.alerts.filter(alert => {
        ...
        if (activeAlertsOnly && !isAlertActive(alert)) return false;
