...
function SearchPageContent() {
  ...
  const clearFilters = () => {
    ...
    setAlertStatus('all');
    setAlertOrganization('all');
    // Ferry filters
    setFerryDay('all');
    setFerryRoute('all');
    setFerryTimeOfDay('all');
    // Event filters
    setEventOrganization('all');
    setRegistrationRequired('all');
  ...
  const filterResults = () => {
    ...
    // Alert organization filtering
    if (alertOrganization !== 'all') {
      filtered.alerts = filtered.alerts.filter(alert =>
        alert.on_behalf_of_organization?.toLowerCase().includes(alertOrganization.toLowerCase())
      );
    }
    ...
    if (eventOrganization !== 'all') {
      filtered.events = filtered.events.filter(event =>
        event.organizer_organization?.toLowerCase().includes(eventOrganization.toLowerCase()) ||
        event.organizer_name?.toLowerCase().includes(eventOrganization.toLowerCase())
      );
    }
    if (registrationRequired !== 'all') {
      filtered.events = filtered.events.filter(event =>
        registrationRequired === 'yes' ? event.registration_required === true : event.registration_required !== true
      );
    }
    if (hasAccessibility) {
      filtered.events = filtered.events.filter(event =>
    ...
    if (wheelchairAccessible) {
      filtered.directory = filtered.directory.filter(business => business.wheelchair_accessible === true);
    }
    if (acceptsCredit) {
      filtered.directory = filtered.directory.filter(business => business.accepts_credit === true);
    }
    if (offersDelivery) {
      filtered.directory = filtered.directory.filter(business => business.offers_delivery === true);
    }
    if (petFriendly) {
      filtered.directory = filtered.directory.filter(business => business.pet_friendly === true);
    }
    if (hasWifi) {
      filtered.directory = filtered.directory.filter(business => business.wifi_available === true);
    }
    if (hasParking) {
            ...
            {activeScope === 'events' && (
              <div>
                ...
                <div className="grid md:grid-cols-2 gap-4">
                  ...
                  <div>
                    <label className="block text-sm text-gray-600 mb-1">Registration</label>
                    <select
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                  ...
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input
                      ...
                      className="w-4 h-4 text-gabriola-green rounded focus:ring-gabriola-green"
                    />
                    <span className="text-sm text-gray-700">💳 Accepts Credit</span>
                  </label>
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input
                      type="checkbox"
